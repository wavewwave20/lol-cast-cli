...
| 중계 | 휠/`↑↓`/`PgUp` 스크롤 · `d` 선수 상세 · `f` 자동스크롤 · `End` 맨 아래로 · `+`/`-` 배속(리플레이) · `q` 뒤로 · `c` 즉시 종료+클리어 |
...
`킬` `퍼블`(퍼스트 블러드) `처형` `용` `바론` `타워` `억제기` `골드` `종료/중지/재개`
...
중계 중 `d`를 누르면 스코어보드 아래에 선수별 상세가 펼쳐진다:

```
      선수                    KDA    Lv    CS   골드   딜%   킬%   와드
탑    Doran(제이스)         2/1/3    14   210  10.2k   23%   50%    8/2
정글  Oner(트런들)          4/0/6    13   155   9.8k   18%   71%   16/5
...
```

KDA · 레벨 · CS · 획득 골드 · 팀 내 딜 지분 · 킬 관여율 · 와드 설치/제거.
